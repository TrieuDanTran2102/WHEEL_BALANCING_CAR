...
MPU6050_t MPU6050;
Motor_TypeDef MT;
PID_TypeDef PID_Angle, PID_Velocity;

double Angle, Rotational_Speed;
double Angle_Output, Velocity_Output;
double Angle_Setpoint = 0, Velocity_Setpoint = 20;

float Velocity_kp = 0.0; //0.0
float Velocity_ki = 1.0; //9.0
float Velocity_kd = 0.0; //0.01
...
//		printf("Speed: %s\r\n", sPart);
//
//		Motor_Controller(parseMotor(mPart), parseDirection(dPart), atoi(sPart), atoi(sPart));
//
//		USB_ReceiveFlag = 0;
//	  }
...
//
//	  		  setTunings(&PID, (float)atoi(pPart), (float)atoi(iPart), (float)atoi(dPart));
//	  	  }
//		  if (strcmp(OP, "ESP32") == 0)
//	  	  {
//			  mPart = strtok(NULL, " ");
//			  dPart = strtok(NULL, " ");
//			  sPart = strtok(NULL, " ");
